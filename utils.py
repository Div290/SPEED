...
def extract_src_x_y(squad_data):
    src_x = []
    src_y = []
    ...
    for article in squad_data['data']:
        for paragraph in article['paragraphs']:
            ...
            for qa in paragraph['qas']:
                ...
                # Check if the question is answerable or not
                ...
                src_x.append(context + " [SEP] " + question + " [SEP] " + answer)
                src_y.append(label)
...
def CSV2Array_yelp(path):
    data = pd.read_csv(path, encoding='latin')
    reviews, labels = data.iloc[:, 1].values.tolist(), data.iloc[:, 0].map({1: 0, 2: 1}).tolist()
    return reviews, labels
